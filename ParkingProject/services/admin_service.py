...
def mostrar_datos_plazas_parking(parking):
    parking.mostrar_todas_las_plazas()  # DESARROLLO


#    for p in parking.plazas: PRODUCCION
...
def consultar_cobros_abonos(parking):
    if not parking.abonados:
        ...
        for a in parking.abonados:
            print(
                f'Abonado: {a.nombre}\t\tDNI:{a.dni}\t\tAbono: {a.tipo_abono}\t\tPrecio: {a.cobro.precio}\t\tFecha Caducidad {a.fecha_caducidad_abono}')
            sum += a.cobro.precio
...
def consultar_cobros_no_abonos(parking):
    while (True):
        try:
            ...
            print("Ha ocurrido un error, introduzca un número válido.")
        else:
            break
        ...
        try:
            ...
            for a in parking.abonados:
                if id_plaza == a.plaza.id_plaza and pin == a.plaza.pin:
                    ...
                    if op == 2:
                        tipo_abono = int(
                            input('Introduzca cuánto tiempo quiere ampliar su abono(SELECCIONE 1, 2, 3 O 4):'
                                  '\n1. Un mes'
                                  '\n2. Tres meses'
                                  '\n3. Seis meses'
                                  '\n4. Un año'))
...
def dar_de_baja_abono(parking):
    while (True):
        try:
            ...
            pin = int(input('Introduzca el pin asociado a su abono:'))
            for a in parking.abonados:
                if a.plaza.id_plaza == id_plaza and pin == a.plaza.pin:
                    parking.abonados.remove(a)
                    del (a)
                else:
                    print('El abonado no existe')
        except:
            print("Ha ocurrido un error, introduzca un pin o id de la plaza válido.")
        else:
            break
    ...
    while (True):
        try:
            ...
            if 1 <= mes_inicio <= 12:
                ...
                print('Ha introducido un mes no válido.')
        except:
            print("Ha ocurrido un error, introduzca un mes o año válido.")
        else:
            break
